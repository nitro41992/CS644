...
public class AverageTaxiTime {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, LongWritable> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (isInteger(column[19]) && isInteger(column[20])) {
				...
				context.write(origin, new LongWritable(Integer.parseInt(column[19])));

				Text destination = new Text("* " + column[17]);
				context.write(destination, new LongWritable(Integer.parseInt(column[20])));
	...
	public static class Reduce extends Reducer<Text, LongWritable, Text, Text> {
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			...
			context.write(new Text("Airport with Lowest average:  "), null);
	...
	public static void main(String[] args) throws Exception {
		...
		Job wordFreq = Job.getInstance(conf,
				"this Program provide 3 airports with the longest and shortest average taxi time per airport (both in and out)");
